...
- 📧 **メール送信**: 処理完了通知の自動メール送信
  - 送信ワーカーの多重起動ガードを実装（開発リロード・重複起動での二重送信を抑止）
  - 簡易デデュープを実装（同一宛先+同一ファイル名の短時間重複送信を防止）
...
| `PORT` | サーバーポート | `5003` |
| `EMAIL_DEDUPE_WINDOW_SECONDS` | メール重複送信の抑止TTL（秒） | `600` |
...
4. **メールが2通届く（重複送信）**
   - 本番で `FLASK_DEBUG=False` になっているか確認
   - 同一音源・同一宛先で短時間に再実行した場合は、デデュープでスキップされます
   - 必要に応じて `EMAIL_DEDUPE_WINDOW_SECONDS` を調整（既定600秒）
