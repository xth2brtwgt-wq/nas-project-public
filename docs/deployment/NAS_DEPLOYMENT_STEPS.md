...
```bash
# ローカル（Mac）から実行
ssh YOUR_USERNAME@[NASのIPアドレス]

# または
ssh YOUR_USERNAME@nas.local
```
...
各プロジェクトで `.env.local` ファイルを作成し、実際の値を設定します。
...
# データフォルダを作成（NAS固有）
sudo mkdir -p /home/YOUR_USERNAME/nas-project-data/document-automation/{uploads,processed,exports,cache,db}
sudo chown -R YOUR_USERNAME:admin /home/YOUR_USERNAME/nas-project-data/document-automation
